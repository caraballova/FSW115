function blah(e) {
    ...
    for (i = 0; i < x.length; i++) {
        if (x[i].type === "checkbox") {
            ...
            axios.put(`https://api.vschool.io/vanessacaraballo/todo/${id}`, y)
...
todoForm.addEventListener("submit", function(event){
    ...
    axios.post("https://api.vschool.io/vanessacaraballo/todo", newTodo)
...
todolist.addEventListener("click", function (e){
    ...
    for(i = 0; i < deleteBtn.length; i++){
      if(e.target == deleteBtn[i]){
        ...
        let id = x.id
        axios.delete(`https://api.vschool.io/vanessacaraballo/todo/${id}`)
